...
import { _sync } from './sync'; // {{{1

import { linear   } from './paths/linear'; // {{{1

import { stepped  } from './paths/stepped'; // {{{1

import { bars     } from './paths/bars'; // {{{1

import { monotoneCubic     as spline  } from './paths/monotoneCubic'; // {{{1

import { catmullRomCentrip as spline2 } from './paths/catmullRomCentrip'; // {{{1

import { addGap, clipGaps, moveToH, moveToV, arcH, arcV, orient, pxRoundGen } from './paths/utils'; // {{{1

function log(name, args) { // {{{1
	console.log.apply(console, [name].concat(Array.prototype.slice.call(args)));
} // }}}1

const cursorPlots = new Set();

function invalidateRects() { // {{{1
...
function setDefaults(d, xo, yo, initY) { // {{{1
	let d2 = initY ? [d[0], d[1]].concat(d.slice(2)) : [d[0]].concat(d.slice(1));
	return d2.map((o, i) => setDefault(o, i, xo, yo));
}

function setDefault(o, i, xo, yo) { // {{{1
	return assign({}, (i == 0 ? xo : yo), o);
}

function snapNumX(self, dataMin, dataMax) { // {{{1
	return dataMin == null ? nullNullTuple : [dataMin, dataMax];
} // }}}1

const snapTimeX = snapNumX;

// this ensures that non-temporal/numeric y-axes get multiple-snapped padding added above/below {{{1
// TODO: also account for incrs when snapping to ensure top of axis gets a tick & value
function snapNumY(self, dataMin, dataMax) {
	return dataMin == null ? nullNullTuple : rangeNum(dataMin, dataMax, 0.1, true);
}

function snapLogY(self, dataMin, dataMax, scale) { // {{{1
	return dataMin == null ? nullNullTuple : rangeLog(dataMin, dataMax, self.scales[scale].log, false);
} // }}}1

const snapLogX = snapLogY;

function snapAsinhY(self, dataMin, dataMax, scale) { // {{{1
	return dataMin == null ? nullNullTuple : rangeAsinh(dataMin, dataMax, self.scales[scale].log, false);
} // }}}1

const snapAsinhX = snapAsinhY;

// dim is logical (getClientBoundingRect) pixels, not canvas pixels {{{1
...
export default function uPlot(opts, data, then) {
	...
	function getHPos(val, scale, dim, off) { // {{{2
		let pct = getValPct(val, scale);
		return off + dim * (scale.dir == -1 ? (1 - pct) : pct);
	}

	function getVPos(val, scale, dim, off) { // {{{2
		let pct = getValPct(val, scale);
		return off + dim * (scale.dir == -1 ? pct : (1 - pct));
	}

	function getPos(val, scale, dim, off) { // {{{2
